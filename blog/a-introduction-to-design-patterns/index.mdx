...
author: Aaron
date: 2022-01-31T22:00:00+09:00
updateDate: 2022-01-31T22:00:00+09:00
...
앞선 [객제지향 프로그래밍](/about) 글을 잘 읽었다면, 객체지향 코드는 Class 보다 Interface 를 단위로 구성된다는것을 이미 알고있을것이다. **'어떤 작업을 수행할지'**는 Interface 로 명시한 채, **'어떻게 작업할지'**는 원하는 구현 클래스(Concrete Class) 를 적재적소에 주입하여 실제 작업을 수행한다.
...
왜 [객제지향 프로그래밍](/about)에선 **상속(extends)보다 구성(composite)을 사용**하라고 할까? 다시 한번 복습하자면
...
이로써 앞서 배운 [객제지향 프로그래밍](/about)의 제 1 원칙 **"'주입': 구현이 아닌 인터페이스에 맞춰 프로그래밍 한다."**와 제 2 원칙 **"상속 대신 구성과 인터페이스를 사용한다"** 을 다시 한번 되새김질 할 수 있었다.
