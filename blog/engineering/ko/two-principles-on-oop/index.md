...
image: {
	url: ./composition-vs-inheritance.png,
	alt: "Composition vs Inheritance"
}
keywords: "객체지향, OOP 원칙, 상속보다 구성, 인터페이스, 다형성, 디자인 패턴"
description: "상속의 한계를 극복하기 위해 인터페이스와 구성을 활용하여 유연하고 확장성 있는 객체지향 코드를 설계하는 두 가지 핵심 원칙을 소개합니다."
---
...
```java
class RedHeadDuck {
	class SuperFlyable; <-- 구현 클래스 // [!code highlight]
	class ShoutQuackable; <-- 구현 클래스 // [!code highlight]
...
```java
class RedHeadDuck {
	interface Flyable = new SuperFlyable(); <-- 인터페이스 (다양한 구현 낄수있음)  // [!code highlight]
	interface Quackable = new ShoutQuackable(); <-- 인터페이스 (다양한 구현 낄수있음)  // [!code highlight]
	doFly() { Flyable.fly() }
	doQuack(){ Quackable.quack() }
}
```
