...
# 아래 내용은 LLM 활용하여 자동생성하도록 한다. Nullable 이긴하나 채우는것이 낫다
# keywords 는 최소 5개 ~ 최대 8개의 단어
# description 은 90자 이내
# image 도 자동 생성할 수 있다면 자동 생성된것으로 사용하자 - 특정 언어나 라이브러리를 대표하는 아이콘과 그 색에 맞춘 배경색 (아마 섬네일 생성기가 인터넷 찾으면 금방 나올것 / 없으면 만들어쓰면됨)
keywords: CORS, SOP, Same-Origin Policy, AJAX, 브라우저 보안, Preflight
description: 웹 브라우저의 보안을 위한 SOP 정책을 보완하여, 안전한 Cross-origin AJAX 호출을 가능하게 하는 CORS 정책의 개념과 검증 절차를 정리합니다.
---
