...
image: { url: ./design-patterns.png, alt: "Design Patterns 개요" }
---

# 디자인 패턴

디자인 패턴은 짧게 **'객체지향 패러다임에서 더 좋은 코드란 무엇인가에 대한 고민의 결과'** 이다. 처음 이 개념은 Gang of Four 라는 4명의 프로그래머가 작성한 [Design Patterns](https://en.wikipedia.org/wiki/Design_Patterns) 책을 통해 세상밖으로 나오게 되었고, 이는 꼭 객체지향 프로그램이이 아니더라도 어느 언어에서도 통용될 수 있는 개발 방식들을 담고 있다.
...
디자인 패턴은 위와 같이 3가지로 구분되어 각각 하위에 정말 다양한 개수의 패턴들이 위치한다. [Refacotring Guru](https://refactoring.guru/design-patterns/catalog) 페이지를 꼭 참조하길 바란다. 실제로 각 카테고리면 어떤 패턴이 존재하는지 볼 수 있고, 각 패턴별로 왜 생겨났고 어떤 개념인지 예제 코드들과 함께 앞으로 학습에 큰 도움이 될것이다.
...
![Implements](./implements.png)

앞선 [객제지향 프로그래밍](/pattern/two-principles-on-oop) 글을 잘 읽었다면, 객체지향 코드는 Class 보다 Interface 를 단위로 구성된다는것을 이미 알고있을것이다. <b>'어떤 작업을 수행할지'</b>는 Interface 로 명시한 채, <b>'어떻게 작업할지'</b>는 원하는 구현 클래스(Concrete Class) 를 적재적소에 주입하여 실제 작업을 수행한다.
...
![Has-A and Implements](./extends2.png)
...
![Openess, when we use Has-A and Implements](./extends3.png)
...
이로써 앞서 배운 [객제지향 프로그래밍](/pattern/two-principles-on-oop)의 제 1 원칙 <b>"'클래스-상속'보다 '인터페이스-구성'을 사용하라"</b>와 제 2 원칙 <b>"'구현 클래스'보다 '인터페이스'로 구성하라"</b>와 을 다시 한번 되새김질 할 수 있었다.
